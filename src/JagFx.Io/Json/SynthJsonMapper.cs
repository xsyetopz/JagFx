...
public static class SynthJsonMapper
{
    ...
    private static VoiceJson VoiceToJson(Voice voice) =>
        new()
        {
            ...
            GapOffEnvelope =
                voice.GapOffEnvelope != null ? EnvelopeToJson(voice.GapOffEnvelope) : null,
            GapOnEnvelope =
                voice.GapOnEnvelope != null ? EnvelopeToJson(voice.GapOnEnvelope) : null,
            Partials =
                voice.Partials.Count > 0 ? voice.Partials.Select(PartialToJson).ToList() : null,
    ...
    private static Voice VoiceFromJson(VoiceJson json) =>
        new(
            ...
            GapOffEnvelope: json.GapOffEnvelope != null
                ? EnvelopeFromJson(json.GapOffEnvelope)
                : null,
            GapOnEnvelope: json.GapOnEnvelope != null ? EnvelopeFromJson(json.GapOnEnvelope) : null,
            Partials: json.Partials != null ? [.. json.Partials.Select(PartialFromJson)] : [],
            Echo: json.Echo != null
    ...
    private static EnvelopeJson EnvelopeToJson(Envelope envelope) =>
        new()
        {
            ...
            Segments = envelope
                .Segments.Select(s => new SegmentJson
                {
                    Duration = s.Duration,
    ...
    private static Envelope EnvelopeFromJson(EnvelopeJson json) =>
        new(
    ...
    private static LfoJson LfoToJson(LowFrequencyOscillator lfo) =>
        new()
        {
    ...
    private static LowFrequencyOscillator LfoFromJson(LfoJson json) =>
        new(
    ...
    private static Filter FilterFromJson(FilterJson json)
    {
        ...
        var modulationEnvelope =
            json.ModulationEnvelope != null ? EnvelopeFromJson(json.ModulationEnvelope) : null;
    ...
    private static FilterCoefficientsJson CoefficientsToJson(
        ImmutableArray<ImmutableArray<ImmutableArray<int>>> coefficients
    ) =>
        new()
        {
